...
import static com.tripi.wallet.TripiApplication.REALM_NAME;
...
public class WearListCallListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if(messageEvent.getPath().contains("/get_history")) {
            ...
            currentAddress = TripiSharedPreference.getInstance().getCurrentAddress(getApplicationContext());
            sendData();
        } else {
            stopSelf();
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final String balance = TripiSharedPreference.getInstance().getBalanceString(WearListCallListenerService.this);
                    final String uncBalance = TripiSharedPreference.getInstance().getUnconfirmedBalanceString(WearListCallListenerService.this);
                    final String address = TripiSharedPreference.getInstance().getCurrentAddress(WearListCallListenerService.this);
                    Gson gson = new Gson();

...
import com.tripi.wallet.ui.activity.splash_activity.SplashActivity;
import com.tripi.wallet.ui.base.base_activity.BaseActivity;
...
import com.tripi.wallet.utils.CustomContextWrapper;
...
import static com.tripi.wallet.ui.base.base_fragment.BaseFragment.BACK_STACK_ROOT_TAG;
...
public class MainActivity extends BaseActivity implements MainActivityView {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null && intent.getAction() != null) {
            switch (intent.getAction()) {
                case TripiIntent.OPEN_FROM_NOTIFICATION:
                    openRootWallet();
                    break;
                default:
                    break;
    ...
    public void resetMenuText() {
        int[] menuResources = new int[]{R.string.wallet, R.string.send, R.string.profile };
    ...
    public boolean checkTouchIdEnable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (TripiSharedPreference.getInstance().isTouchIdEnable(getContext())) {
    ...
    public void recolorStatusBarRed() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            getWindow().setStatusBarColor(ContextCompat.getColor(getContext(), R.color.accent_red_color));
        }
    }
...
//                        break;
        ...
        switch (intent.getAction()) {
            case TripiIntent.SEND_FROM_SDK:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = intent.getStringExtra(TripiIntent.SEND_ADDRESS);
                mAmountForSendAction = intent.getStringExtra(TripiIntent.SEND_AMOUNT);
                mTokenAddressForSendAction = intent.getStringExtra(TripiIntent.SEND_TOKEN);
                break;
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(CustomContextWrapper.wrap(newBase, TripiSettingSharedPreference.getInstance().getLanguage(newBase)));
    }

    public TripiApplication getTripiApplication() {
        return (TripiApplication) getApplication();
    }
    ...
    private int[] blackThemeIcons = {R.drawable.ic_wallet, R.drawable.ic_profile, R.drawable.ic_send};
    private int[] lightThemeIcons = {R.drawable.ic_wallet_light, R.drawable.ic_profile_light, R.drawable.ic_send_light};
    ...
    protected void updateTheme() {
        ...
        if (ThemeUtils.getCurrentTheme(this).equals(ThemeUtils.THEME_DARK)) {
            ...
            recolorStatusBar(R.color.accent_red_color);
        } else {
            int[][] states = new int[][]{
...
//        menu.findItem(R.id.item_news).setIcon(icons[2]);
        menu.findItem(R.id.item_send).setIcon(icons[2]);
    }
    ...
    public void onAuth(){
        registerKeyboardListener();
        showBottomNavigationView(ThemeUtils.currentTheme.equals(ThemeUtils.THEME_DARK)? R.color.background : R.color.accent_red_color);
    }
    ...
    public Realm getRealm(){
        return ((TripiApplication)getApplication()).getRealm();
    }
}
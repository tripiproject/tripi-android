...
import com.tripi.wallet.utils.PinEntryEditText;
...
public class PinDialogFragment extends DialogFragment {
    ...
    @BindView(com.tripi.wallet.R.id.til_wallet_pin)
    PinEntryEditText mWalletPin;

    @BindView(com.tripi.wallet.R.id.tv_toolbar_title)
    FontTextView mTextViewToolBarTitle;

    @BindView(com.tripi.wallet.R.id.tooltip)
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        View view = LayoutInflater.from(getActivity()).inflate(ThemeUtils.getCurrentTheme(getContext()).equals(ThemeUtils.THEME_DARK) ? com.tripi.wallet.R.layout.dialog_fragment_pin : com.tripi.wallet.R.layout.dialog_fragment_pin_light, null);
    ...
    private String getSixDigitPassword() {
        return TripiSharedPreference.getInstance().getSixDigitPassword(getContext());
    }
    ...
    private String getFourDigitPassword() {
        return TripiSharedPreference.getInstance().getPassword(getContext());
    }
            ...
            ((MainActivity) getActivity()).checkSensorState(new MainActivity.SensorStateListener() {
                ...
                public void onRequest(MainActivity.SensorState sensorState) {
                    if (sensorState == MainActivity.SensorState.READY) {
                        mTextViewToolBarTitle.setText(com.tripi.wallet.R.string.confirm_fingerprint_or_pin);
                        prepareSensor();
                    } else {
                        mTextViewToolBarTitle.setText(com.tripi.wallet.R.string.enter_pin);
                    }
                }
            });
        } else {
    ...
    private void confirmError(String errorText) {
        if (getDialog() != null) {
            ...
            tooltip.setTextColor(ContextCompat.getColor(getContext(), com.tripi.wallet.R.color.accent_red_color));
    ...
    private class FingerprintHelper extends FingerprintManagerCompat.AuthenticationCallback {
        ...
        public void onAuthenticationSucceeded(FingerprintManagerCompat.AuthenticationResult result) {
            ...
            String encoded = TripiSharedPreference.getInstance().getTouchIdPassword(getContext());
    ...
    private void confirm(String pin) {
        Long banTime = TripiSharedPreference.getInstance().getBanTime(getContext());
        ...
        if (isCorrect) {
            ...
            confirmError(getContext().getString(com.tripi.wallet.R.string.incorrect_pin));
        }
    }
    ...
    private void changeBanState(boolean isCorrect) {
        ...
        if (isCorrect) {
            ...
            failedAttemptsCount = TripiSharedPreference.getInstance().getFailedAttemptsCount(getContext());
            failedAttemptsCount++;
            if (failedAttemptsCount == 3) {
                Long currentTime = Calendar.getInstance().getTimeInMillis();
                Long banTime = currentTime + mBanTime;
                TripiSharedPreference.getInstance().setBanTime(getContext(), banTime);
                failedAttemptsCount = 0;
                mPinCallBack.onError(getString(R.string.sorry_please_try_again_in) + " " + mBanTime.intValue() / 60000 + " " + getString(R.string.minutes));
            }
        }
        TripiSharedPreference.getInstance().setFailedAttemptsCount(getContext(), failedAttemptsCount);
    }
